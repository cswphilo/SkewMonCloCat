...
Finally, we finally consider a deductive system for a skew version of non-commutative intuitionistic linear logic with additive conjunction and disjunction.
...
Tags of the form $\# \Gamma$ are used to record different splitting of the context in applications of $\lleft$, while tag $\bullet$ is used to mark when rule $\lleft$ sends tagged formulae to the left premis and when rule $\pass$ moves a tagged formula to the stoup.
...
\item \niccolo{Talk about the difference in passivation}
\item The side condition in rule $\lleft$ should be read as follows. If $\Delta$ is empty, then the sequent is either untagged (so there is no $t$) or the tag $t$ is equal to $\# \Gamma$. If $\Delta$ is non-empty, then $t = \bullet$.
...
Proving completeness of the extended focused sequent calculus is more involved. In fact, the complication resides in stating and proving the analog of Proposition \ref{prop:GenRightRules}. First, define an operation $\mathsf{impconj}(A)$ which produces two lists of formulae as follows: \niccolo{give definition}. The statement of Proposition \ref{prop:GenRightRules} for focused sequent calculus in (\ref{eq:focus:impl}) the then becomes:
