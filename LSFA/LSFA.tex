...
We introduce a congruence relation $\circeq$ on the sets of cut-free derivations. The list of its generating equations is given in Figure~\ref{fig:circeq}.
\begin{figure}[t]
  \small
  \begin{equation*}
  \begin{array}{rlll}
...
When oriented from left-to-right, the equations in Figure~\ref{fig:circeq} become a  rewrite system, which is confluent with unique normal forms. Here we provide an explicit description of the proof normal forms of (\ref{eq:seqcalc}).
...
There are four tags: $\tP , \tCone , \tCtwo , \tE$. Intuitively, they respectively correspond to rules $\pass , \andlone , \andltwo$ and all the remaining non-invertible rules.
...
A list of tags $l$ is \emph{valid} if it is non-empty and either $(i)$ $\tE \in l$ or $(ii)$ both $\tCone \in l$ and $\tCtwo \in l$.
%%  \begin{itemize}
...
\begin{proof}
  ...
  \begin{itemize}
    ...
    \item If $A = A' \land B'$, then there exists $fs_1 : [T \mid \Gamma \vdash^{t_i}_{\F} A'_i]_{i \in [1 , \dots , m]}$, $l_1 = t_1 , \dots , t_m$, $fs_2 : [T \mid \Gamma \vdash^{t_i}_{\F} B'_i]_{i \in [m+1 , \dots , n]}$, and $l_2 = t_{m+1} , \dots , t_n$ where $fs$ is the concatenation of $fs_1$ and $fs_2$ and $l$ is the concatenation of $l_1 $ and $l_2$.
...
\begin{proof}
  ...
  \item If $f_1 = \F 2 \LI \,f'_1$, then all the derivations in $fs'$ necessarily end with $\F 2 \LI$ as well. %In this case, we apply $\F 2 \LI$ to all elements in $fs$ and
...
%          \quad
  ...
  % \end{itemize}
\end{proof}
Finally, a right non-invertible rule in $\RI$ is defined as follows: we invert its premise (for $\tr^{\RI}$, only the left premise), and then we apply the corresponding generalized version.
% For example:
% \begin{displaymath}
%   \begin{array}{c}
%     \proofbox{
