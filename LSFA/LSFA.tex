...
%  \end{itemize}
  ...
  % \end{itemize}
\end{proof}
Finally, a right non-invertible rule in (\ref{eq:admis}) is defined as follows: first invert its premises (for $\tr^{\RI}$, only the left premise) using Lemma \ref{lem:RI:invert}. Then apply the corresponding generalized version of the rule in Theorem \ref{theorem:focus:sound:complete}.
% For example:
% \begin{displaymath}
%   \begin{array}{c}
%     \proofbox{
...
\end{theorem}
\begin{proof}
  The first bullet is proved by structural induction on the given equality proof $e : f \circeq g$. The other bullets are proved by structural induction on $f$. See the associated Agda formalization for more details.
\end{proof}
