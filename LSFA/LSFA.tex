  ...
  \begin{itemize}
    ...
    \begin{itemize}
      ...
      \begin{displaymath}
        ...
        \end{array}
      \end{displaymath}
      Note that $fs^{*'}$ is a list of sequents which are tagged version of those in $fs^{*}$.
...
Sequent calculus (\ref{eq:seqcalc}) extended with $\ex$ is an internal language of skew symmetric monoidal category with additive connectives since $\ex$ characterizes the natural isomorphism $s_{A , B , C} : A \ot (B \ot C) \to A \ot (C \ot B)$ which represents skew symmetry in \cite{bourke:lack:braided:2020}.
...
\subsection{Implication}\label{subsec:impl}
implication, incorporate tags of tracking new formulae and monitoring proof process.
