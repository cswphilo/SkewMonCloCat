...
Moreover, $\orl^{\mathsf{C}}$ should be forbidden since it would make some inadmissible sequents provable in the sequent calculus. 
...
\begin{itemize}
  ...
  \item[($\vdash^{t}_{\LI}$)] Since the stoup is either empty or a negative formula, we switch to phase $\Pass$ immediately.
...
\begin{lemma}
  \begin{displaymath}
    \infer[\andr^{*}_t]{T \mid \Gamma \vdash^{l}_{\RI} A}{
      \deduce{[T \mid \Gamma \vdash^{t_i}_{\Pass} A_i]_{i \in [1 , \dots , n]}}{fs}
    }
  \end{displaymath}
...
\begin{proof}
  ...
  \begin{itemize}
    \item If $A$ is a positive formula, we have $fs = [ f ]$ and $\andr^*_{t} \text{ } fs = \Pass 2 \LI \text{ } (\LI 2 \RI \text{ } f)$.
      ...
      \begin{array}{cc}
        ...
        \infer[\andr]{T \mid \Gamma \vdash^{l_1 , l_2}_{\RI} A' \land B'}{
          \infer[\andr^{*}_{t}]{T \mid \Gamma \vdash^{l_1}_{\RI} A'}{
            \deduce{[T \mid \Gamma \vdash^{t_i}_{\Pass} A'_i]_{i \in [1 , \dots , m_1]}}{fs_1}
          }
          &
          \infer[\andr^{*}_{t}]{T \mid \Gamma \vdash^{l_2}_{\RI} B'}{
            \deduce{[T \mid \Gamma \vdash^{t_i}_{\Pass} B'_i]_{i \in [1 , \dots , m_2]}}{fs_2}
          }
        }
       }
      \end{array}
