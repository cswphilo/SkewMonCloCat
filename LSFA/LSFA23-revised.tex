...
For example, consider the following proof of sequent $- \mid \I \ot X \vdash_\F ((I \ot X) \land (\I \ot X)) \lor Y$: % in (\ref{eq:seqcalc}): 
...
%because of the case when the derivation ends with $\andr$.
...
The sequent calculus in (\ref{eq:seqcalc}) can be made ``fully'' additive by including two units $\top$ and $\bot$ (the latter named 0 in linear logic literature), %corresponding to $\land$ and $\lor$ respectively
