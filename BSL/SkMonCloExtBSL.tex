...
%\begin{proof}
  ...
  \begin{equation*}\label{eq:admis:sym}
    ...
    \\[6pt]
      ...
      \end{array}
    \end{equation*}
  From the admissibility of these rules, we can define a focusing function $\mathsf{focus} : S \mid \Gamma \vdash A \to S \mid \Gamma \spl \quad \vdash_{\C} A$ by induction on the input derivation. % in \ref{eq:seqcalc:sym}.
%   \begin{align*}
% 	\ax &\defeq \ax^{\C}
% 	\\
% 	\unitr &\defeq \unitr^{\C}
...
%\end{proof}
  Similarly to the non-commutative case, the main challange for the construction of $\mathsf{focus}$ lies in proving the admissibility of $\tr^{\C}$, the right $\ot$-rule in phase $\C$. 
