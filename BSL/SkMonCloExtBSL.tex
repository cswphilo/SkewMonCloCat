...
%% 	\]
	...
	Following the idea in the study, we consider a symmetric extension (\SkMILL) of \SkNMILL \text{ }\ref{eq:seqcalc} by adding an exchange rule where only formulae in context are exchangeable.
	...
	(logic as resources interpretation of ex)
...
Categorical correspondent to symmetric extension of \SkNMILL \text{ }is symmetric skew monoidal closed category, where skew symmetric isomorphism $s_{A , B , C} : (A \ot B) \ot C \to (A \ot C) \ot B$ and equations below are added to the definition \ref{def:skewcat}.
\begin{equation*}
	\footnotesize
	\def\arraystretch{2.5}
	\begin{array}{lc}
