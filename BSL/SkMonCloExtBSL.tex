...
\begin{document}
	...
	%In this sense, two equivalent proofs in \SkNMILL\ become identical in focused calculus.
	 ...
	 To prove the admissibility of $\tr_{\Gamma'}^{\RI}$ by induction, the first thing is to prove $\lright^{*}$ in \ref{eq:inter:ante} is admissible in $\vdash^{x}_\RI$.
	 \begin{fact}
		\begin{displaymath}
	 ...
	 \begin{proof}
		...
		\begin{itemize}
			\item $\Delta = [ \text{ } ]$, then $\lright^{*} \text{ } f = f$;
			\item $\Delta = A , \Delta'$, then $\lright^{*} f = \lright \text{ } (\lright^{*} \text{ } f)$.
		\end{itemize}
	 \end{proof}
	 \begin{fact}
		\begin{displaymath}
	 ...
	 \begin{proof}
		...
		\begin{displaymath}
			...
			\begin{array}{c}
				...
				\qquad
				\infer[\tr_{\Gamma'}^{\LI}]{S \mid \Gamma , \Delta \vdash_{\LI} \ldbc \Gamma' \mid A \rdbc_{\lolli} \ot B}{
					\deduce{S \mid \Gamma , \Gamma' \vdash_{\LI} A}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
			\\[6pt]
				\infer[\tr_{\Gamma'}^{\Pass}]{S \mid \Gamma , \Delta \vdash_{\Pass} \ldbc \Gamma' \mid A \rdbc_{\lolli} \ot B}{
					\deduce{S \mid \Gamma , \Gamma' \vdash_{\Pass} A}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
				\qquad
				\infer[\tr_{\Gamma'}^{\F}]{S \mid \Gamma , \Delta \vdash_{\F} \ldbc \Gamma' \mid A \rdbc_{\lolli} \ot B}{
					\deduce{S \mid \Gamma , \Gamma' \vdash_{\F} A}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
			\end{array}
		\end{displaymath}
		% The idea is that we push induction up from phase to phase until $\tr_{\Gamma'}^{\F}$ is proved only with the use of rules in focused calculus and admissible rules independent from mutual induction.
		% \begin{displaymath}
		% 	\begin{array}{lc}
		% 		(\tr_{\Gamma'}^{\RI}) &
		% 		\proofbox{\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } f , g) \defeq \tr_{\Gamma' , A'}^{\RI} \text{ } (f , g)
		% 		\qquad
		% 		\tr_{\Gamma'}^{\RI} \text{ } (\LI 2 \RI \text{ } f , g) \defeq \LI 2 \RI \text{ } (\tr_{\Gamma'}^{\LI} \text{ } (f , g))
		% 		}
		% 	\end{array}
		% \end{displaymath}
		There are two cases for $\tr_{\Gamma'}^{\RI}$:
		\begin{enumerate}
			\item $\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } f , g) \defeq \tr_{\Gamma' , A'}^{\RI} \text{ } (f , g)$ and 
		...
		\begin{enumerate}
			\item $\tr_{\Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g) \defeq \F 2 \Pass \text{ } (\tr \text ( \lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\pass \bullet \text{ } f))), g))$ when passivated formula is in $\Gamma'$,
