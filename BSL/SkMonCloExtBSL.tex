...
\\[6pt]
	 ...
	 To prove the admissibility of $\tr_{\Gamma'}^{\RI}$, a focused variant of $\lright^{*}$ is essential.
	 \begin{proposition} The following rule is admissible:
		\begin{displaymath}
			\infer[\lright^{*}]{S \mid \Gamma \vdash^{x}_{\RI} \ldbc \Delta \mid C\rdbc_{\lolli} }{
				\deduce{S \mid \Gamma , \Delta^x \vdash^{x}_{\RI} C}{f}
			}
		\end{displaymath}
	 \end{proposition}
	 \begin{proof}
		...
		\begin{itemize}
			\item if $\Delta = [ \text{ } ]$, then $\lright^{*} \text{ } f \defeq f$;
	 ...
	 \end{proposition}
	 \begin{proof}
		Proof is done by mutual induction on left premises of following rules.
		\begin{equation}\label{eq:mutual:induction:gen-otR}
			...
			\begin{array}{c}
				...
				\infer[\tr_{\Gamma'}^{\LI}]{S \mid \Gamma , \Delta \vdash_{\LI} \ldbc \Gamma' \mid P \rdbc_{\lolli} \ot B}{
					\deduce{S \mid \Gamma , \Gamma' \vdash_{\LI} P}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
			\\[6pt]
				\infer[\tr_{\Gamma'}^{\Pass}]{T \mid \Gamma , \Delta \vdash_{\Pass} \ldbc \Gamma' \mid P \rdbc_{\lolli} \ot B}{
					\deduce{T \mid \Gamma , \Gamma' \vdash_{\Pass} P}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
				\qquad
				\infer[\tr_{\Gamma'}^{\F}]{T \mid \Gamma , \Delta \vdash_{\F} \ldbc \Gamma' \mid P \rdbc_{\lolli} \ot B}{
					\deduce{T \mid \Gamma , \Gamma' \vdash_{\F} P}{f}
					&
					\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
				}
			\end{array}
		\end{equation}
		There are two cases for $\tr_{\Gamma'}^{\RI}$:
		\begin{enumerate}
			\item $\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } f , g)$
			...
			\begin{align*}
                ...
                \begin{array}{cc}
                    ...
                    &
                    \proofbox{
					\infer[\tr^{\RI}_{\Gamma' , A}]{S \mid \Gamma , \Delta \vdash_{\RI} \ldbc \Gamma' \mid (A \lolli B) \rdbc_{\lolli} \ot C}{
						\deduce{S \mid \Gamma , \Gamma' , A \vdash_{\RI} B}{f}
						&
						\deduce{{-} \mid \Delta \vdash_{\RI} C}{g}
					}
				...
				\end{array}
			\end{align*}
			where by definition $\ldbc \Gamma' \mid (A \lolli B) \rdbc_{\lolli} = \ldbc \Gamma' , A \mid B \rdbc_{\lolli} $.
			\item $\tr_{\Gamma'}^{\RI} \text{ } (\LI 2 \RI \text{ } f , g) \defeq \LI 2 \RI \text{ } (\tr_{\Gamma'}^{\LI} \text{ } (f , g))$.
		\end{enumerate}
		Three cases for $\tr_{\Gamma'}^{\LI}$:
		\begin{enumerate}
			\item $\tr_{\Gamma'}^{\LI} \text{ } (\unitl \text{ } f , g) \defeq \unitl \text{ } (\tr_{\Gamma'}^{LI} \text{ } (f , g))$, 
			\item $\tr_{\Gamma'}^{\LI} \text{ } (\tl \text{ } f , g) \defeq \tl \text{ } (\tr_{\Gamma'}^{LI} \text{ } (f , g))$, and
			\item $\tr_{\Gamma'}^{\LI} \text{ } (\Pass 2 \LI \text{ } f , g) \defeq \Pass 2 \LI \text{ } (\tr_{\Gamma'}^{LI} \text{ } (f , g))$.
		\end{enumerate}
		$\tr_{\Gamma'}^{\Pass}$ is a bit more complicated since the passivated formula could appear in $\Gamma'$:
		\begin{enumerate}
			\item When passivated formula is in $\Gamma'$, $\tr_{A , \Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g)$,
			...
			\begin{displaymath}
			  ...
			  \end{array}
			\end{displaymath}
			\item $\tr_{\Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g) \defeq \pass \text{ } (\tr_{\Gamma'}^{\Pass} \text{ } (f , g))$, and
			\item $\tr_{\Gamma'}^{\Pass} \text{ } (\F 2 \Pass \text{ } f , g) \defeq \F 2 \Pass \text{ } (\tr_{\Gamma'}^{\F} \text{ } (f , g)) $.
		\end{enumerate}
		Finally, there are five cases for $\tr_{\Gamma'}^{\F}$:
		\begin{enumerate}
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\ax , g) \defeq \tr \text{ } (\ax , g)$ where $\Gamma'$ is empty.
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\unitr , g) \defeq \tr \text{ } (\unitr , g)$ where $\Gamma'$ is empty.
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\tr \text{ } (f , f') , g)$
		  % \defeq \tr \text{ } (\lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{} (\F 2 \Pass \text{ } (\tr \text{ } (f , f'))))) , g)
		  \begin{displaymath}
			\footnotesize
			\begin{array}{cc}
			% \infer[\tr_{\Gamma' , \Delta}^{\F}]{T \mid \Gamma , \Lambda \vdash_{\F} \ldbc \Gamma' , \Delta \mid A \ot B \rdbc_{\lolli} \ot C}{
			% 	\infer[\tr]{T \mid \Gamma , \Gamma' , \Delta \vdash_{\F} A \ot B}{
			% 		\deduce{T \mid \Gamma , \Gamma' \vdash^{\bullet}_{\RI} A}{f}
			% 		&
            ...
            &
            \proofbox{
			\infer[\tr]{T \mid \Gamma , \Lambda \vdash_{\F} \ldbc \Gamma' , \Delta \mid A \ot B \rdbc_{\lolli} \ot C}{
				\infer[\lright^{*}]{T \mid \Gamma \vdash^{\bullet}_{\RI} \ldbc \Gamma' , \Delta \mid A \ot B \rdbc_{\lolli}}{
					\infer[\mathsf{sw}]{T \mid \Gamma , \Gamma'^{\bullet} , \Delta^{\bullet} \vdash^{\bullet}_{\RI} A \ot B}{
						\infer[\tr]{T \mid \Gamma , \Gamma'^{\bullet} , \Delta^{\bullet} \vdash^{\bullet}_{\F} A \ot B}{
							\deduce{T \mid \Gamma , \Gamma' \vdash^{\bullet}_{\RI} A}{f}
							&
							\deduce{{-} \mid \Delta \vdash_{\RI} B}{f'}
						}
		    ...
		    \end{array}
		  \end{displaymath}
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f , f') , g) \defeq \lleft \text{ } (f , \tr_{\Gamma'}^{\LI} \text{ } (f' , g))$ when there is no formula in $\Gamma'$ comes from $f$.
		  ...
		  \begin{displaymath}
			\footnotesize
		  \begin{array}{cc}
			% \infer[\tr_{\Gamma'_{0} , D , \Gamma'_1 , \Delta}^{\F}]{A' \lolli B' \mid \Gamma , \Lambda \vdash_{\F} \ldbc \Gamma'_{0} , D , \Gamma'_1 \mid P \rdbc_{\lolli} \ot B}{
			% 	\infer[\lleft]{A' \lolli B' \mid \Gamma , \Gamma'_0 , D , \Gamma'_1 , \Delta \vdash_{\F} P}{
			% 		\deduce{{-} \mid \Gamma , \Gamma'_0 , D , \Gamma'_1  \vdash_{\RI} P}{f}
			% 		&
			% 		\deduce{B' \mid \Delta \vdash_{\LI} B}{f'}
			% 	}
			% 	&
            ...
            &
            \proofbox{
			\infer[\tr]{A' \lolli B' \mid \Gamma , \Lambda \vdash_{\F} \ldbc \Gamma'_{0} , D , \Gamma'_1 , \Delta \mid P \rdbc_{\lolli} \ot B}{
				\infer[\lright^{*}]{A' \lolli B' \mid \Gamma \vdash^{\bullet}_{\RI} \ldbc \Gamma'_{0} , D , \Gamma'_1 , \Delta \mid P \rdbc_{\lolli}}{
					\infer[\mathsf{sw}]{A' \lolli B' \mid \Gamma , \Gamma'^{\bullet}_{0} , D^{\bullet} , \Gamma'^{\bullet}_{1} , \Delta^{\bullet} \vdash^{\bullet}_{\RI} P}{
						\infer[\lleft]{A' \lolli B' \mid \Gamma , \Gamma'^{\bullet}_{0} , D^{\bullet} , \Gamma'^{\bullet}_{1} , \Delta^{\bullet} \vdash^{\bullet}_{\F} P}{
							\deduce{{-} \mid \Gamma , \Gamma'_0 , D , \Gamma'_1  \vdash_{\RI} P}{f}
							&
							\deduce{B' \mid \Delta \vdash_{\LI} B}{f'}
						}
	...
	Following the idea in the study, we consider a symmetric extension (\SkMILL) of \SkNMILL \text{ }\ref{eq:seqcalc} by adding an exchange rule only swapping formulae in context.
...
In \SkMILL, only formulae in context can be swapped, so it naturally captures the skew exchange natural isomorphism ($s_{A , B , C} : (A \ot B) \to C \ot (A \ot C) \ot B$) in symmetric skew monoidal closed category and in the meantime, stays away from proving normal exchange isomorphism ($s_{A , B} : A \ot B \to B \ot A$).
\begin{displaymath}
    \footnotesize
\begin{array}{cc}
    (s_{A , B ,C}) & (s_{X , Y})
    \\[10pt]
    \proofbox{
        \infer[\tl]{(A \ot B) \ot C \mid \quad \vdash (A \ot C) \ot B}{
    ...
    &
    \proofbox{
        \infer[\tl]{X \ot Y \mid \quad \vdash Y \ot X}{
            \infer[\tr]{X \mid Y \vdash Y \ot X}{
                \deduce{X \mid Y \vdash Y}{??}
                &
                \deduce{{-} \mid \quad \vdash X}{??}
            }
        }
    }
\end{array}
...
Due to the new $\ex$ rule, \ref{fig:circeq} is extended with following equations related with $\ex$.
\begin{equation}\label{fig:circeq:sym}
	\footnotesize
  \begin{array}{rlll}
    \unitl \text{ } (\ex_{A , B} \text{ } f) &\circeq \text{ } \ex_{A , B} \text{ } (\unitl \text{ } f) &\hspace{-30pt}f: {-} \mid \Gamma , A , B , \Delta \vdash C
    \\
    \pass \text{ } (\ex_{A , B} \text{ } f) &\circeq \text{ } \ex_{A , B} \text{ } (\pass \text{ } f) &\hspace{-30pt}f : A' \mid \Gamma , A , B , \Delta \vdash C
    \\
    \tl \text{ } (\ex_{A , B} \text{ } f) &\circeq \text{ } \ex_{A , B} \text{ } (\tl \text{ } f) &\hspace{-30pt}f : A' \mid B' , \Gamma , A , B , \Delta \vdash C
    \\
    \tr \text{ }(\ex_{A , B} \text{ } f , g) &\circeq \text{ } \ex_{A , B} \text{ } (\tr \text{ } (f , g)) &\hspace{-30pt}f : S \mid \Gamma_0 , A , B , \Gamma_1 \vdash A' , g : {-} \mid \Delta \vdash B'
    \\
    \tr \text{ } (f , \ex_{A , B} \text{ } g) &\circeq \text{ } \ex_{A , B} \text{ } (\tr \text{ } (f , g)) &\hspace{-30pt}f : S \mid \Gamma \vdash A' , g : {-} \mid \Delta_0 , A , B , \Delta_1 \vdash B'
    \\
    \lleft \text{ } (\ex_{A , B} \text{ } f , g) &\circeq \text{ } \ex_{A , B} \text{ } (\lleft \text{ } (f , g)) &\hspace{-30pt}f : {-} \mid \Gamma_0 , A , B , \Gamma_1 \vdash A' , g : B' \mid \Delta \vdash C
    \\
    \lleft \text{ } (f , \ex_{A , B} \text{ } g) &\circeq \text{ } \ex_{A , B} \text{ } (\lleft \text{ } (f , g)) &\hspace{-30pt}f : {-} \mid \Gamma \vdash A' , g : B' \mid \Delta_0 , A , B , \Delta_1 \vdash C
    \\
    \lright \text{ } (\ex_{A , B} \text{ } f) &\circeq \text{ } \ex_{A , B} \text{ } (\lright \text{ } f) &\hspace{-30pt}f : S \mid \Gamma , A , B , \Delta , A' \vdash B'
	\\
	\ex_{A , B} \text{ } (\ex_{A' , B'} \text{ } f) &\circeq \text{ } \ex_{A' , B'} \text{ } (\ex_{A , B} \text{ } f) &\hspace{-30pt}f: S \mid \Gamma , A , B , \Delta , A' , B' , \Lambda \vdash C
	\\
	\ex_{B , A} \text{ } (\ex_{A , B} \text{ } f) &\circeq \text{ } f &\hspace{-30pt}f: S \mid \Gamma , A , B , \Delta \vdash C
	\\
	\ex_{A , B} \text{ } (\ex_{A , D} \text{ } (\ex_{B , D} \text{ } f)) &\circeq \text{ } \ex_{B , D} \text{ } (\ex_{A , D} \text{ } (\ex_{A , B} \text{ } f)) &f : S \mid \Gamma , A , B , D , \Delta \vdash C
  \end{array}
\end{equation}
A more general form of exchange rules is admissible, where a formula is swapped with a list of formulae.
...
\end{proposition}
\begin{proof}
  We only show the proof $\mathsf{exs_{A , \Lambda}}$. The proof of $\mathsf{exs_{\Lambda,A}}$ is similar. The proof proceeds by induction on $\Lambda$.
  If $\Lambda$ is empty, then $\mathsf{exs_{A , }}$ there is nothing we have to do.
...
\end{proof}
Cut admissibility of \ref{eq:seqcalc:sym} is proved with the help of general exchange rules in (\ref{eq:gen-ex}). For instance
\begin{displaymath}
  \footnotesize
  \proofbox{
...
\end{displaymath}
The other case is similar where the cut formula is $B$.
...
\begin{figure}
	...
	\end{array}
\end{equation*}
\caption{Equations related to $s_{A , B , C}$}
\label{fig:sym:skew:mon:clo}
\end{figure}

Categorical correspondent to symmetric extension of \SkNMILL \text{ }is symmetric skew monoidal closed category, where skew symmetric isomorphism $s_{A , B , C} : (A \ot B) \ot C \to (A \ot C) \ot B$ and equations in figure \ref{fig:sym:skew:mon:clo} are added to the definition \ref{def:skewcat}.

The structural law of symmetry is usually in the form $B \ot C \to C \ot B$, in the skew case, however, the leftmost object is fixed.
Therefore, given two objects $(\dots ((A \ot B_1) \ot B_2) \dots)$ and $(\dots ((A \ot B_{i_1}) \ot B_{i_2}) \dots)$, there is always a map between them in the symmetric skew monoidal closed category when $A$ staying in the leftmost position, where $i_1 , \dots , i_n$ is a permutation of $1 , \dots , n$.

Mac Lane's coherence condition for normal symmetric 

1. permutation of skew exchange
2. normal coherence conditions fail here
\subsection{Focused system with tag annotations}
\SkMILL \text{ }also enjoys focusing property, a sub-proof system in which derivations are in normal form corresponding to the equivalence relation $\circeq$.
...
\begin{proof}
  ...
  \begin{equation}\label{eq:admis:sym}
    ...
    \\[6pt]
      ...
      \end{array}
    \end{equation}
  From aforementioned admissible rules, we can define a focusing function $\mathsf{focus} : S \mid \Gamma \vdash A \to S \mid \Gamma \spl \quad \vdash_{\C} A$ by induction on derivations in \ref{eq:seqcalc:sym}.
  \begin{align*}
	\ax &\defeq \ax^{\C}
	\\
	\unitr &\defeq \unitr^{\C}
...
\end{proposition}
\begin{proof}
    We modify rules in \ref{eq:mutual:induction:gen-otR} to fit the new $\lright^{*}$ in current focused caclculus.
    \begin{equation}
        \footnotesize
        \begin{array}{c}
            \\[6pt]
        ...
        \end{array}
    \end{equation}
    Proof is done by mutual induction on left premises. Notice that we use $\Gamma^{x}$ indicating some formulae in $\Gamma$ are tagged.

There are two cases for $\tr_{\Gamma'}^{\RI}$:
\begin{enumerate}
    \item $\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } (\ex \text{ } (\RI 2 \C \text{ } f)) , g)$
    % \defeq \tr_{\Gamma' , A}^{\RI} \text{ } (f , g)
    \begin{displaymath}
        \footnotesize
        \begin{array}{cc}
        ...
        \end{array}
    \end{displaymath}
    $\Gamma_0 ; \Gamma_1 \bowtie (\Gamma' , \Gamma'')$ and $\Gamma_1 \lrwavy \Gamma'_1$ are side conditions of the left hand side and $\Gamma_0 ; (\Gamma_{10} , A , \Gamma_{11}) \bowtie (\Gamma' , A , \Gamma'')$ and
    $(\Gamma_{10} , A , \Gamma_{11}) \lrwavy (\Gamma'_1 , A)$ are of the right hand side, where $(\Gamma_{10} , \Gamma_{11}) = \Gamma_1$.
    \item $\tr_{\Gamma'_1}^{\RI} \text{ } (\LI 2 \RI \text{ } f , g) \defeq \LI 2 \RI \text{ } (\tr_{\Gamma'_1}^{\LI} \text{ } (f , g))$.
\end{enumerate}
Three cases for $\tr_{\Gamma'_1}^{\LI}$:
\begin{enumerate}
    \item $\tr_{\Gamma'_1}^{\LI} \text{ } (\unitl \text{ } f , g) \defeq \unitl \text{ } (\tr_{\Gamma'_1}^{\LI} \text{ } (f , g))$, 
    \item $\tr_{\Gamma'_1}^{\LI} \text{ } (\tl \text{ } f , g) \defeq \tl \text{ } (\tr_{\Gamma'_1}^{\LI} \text{ } (f , g))$, and
    \item $\tr_{\Gamma'_1}^{\LI} \text{ } (\Pass 2 \LI \text{ } f , g) \defeq \Pass 2 \LI \text{ } (\tr_{\Gamma'_1}^{\LI} \text{ } (f , g))$.
\end{enumerate}
$\tr_{\Gamma'_1}^{\Pass}$ is a bit more complicated since the passivated formula could appear in $\Gamma'_1$:
\begin{enumerate}
    \item When passivated formula is in $\Gamma'_1$, then $\tr_{\Gamma'_1}^{\Pass} \text{ } (\pass \text{ } f , g) $ 
    % \defeq \F 2 \Pass \text{ } (\tr \text ( \lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\pass \text{ } f))), g))
    \begin{displaymath}
        \footnotesize
        \begin{array}{ll}
        ...
        \infer[\F 2 \Pass]{{-} \mid \Gamma_0 , \Delta \vdash_{\Pass} \ldbc \Gamma'_1 \mid P \rdbc_{\lolli} \ot B}{
            \infer[\tr]{{-} \mid \Gamma_0 , \Delta \vdash_{\F} \ldbc \Gamma'_1 \mid P \rdbc_{\lolli} \ot B}{
                \infer[\lright^{*}]{{-} \mid \Gamma_0 \vdash^{\bullet}_{\RI} \ldbc \Gamma'_1 \mid P \rdbc_{\lolli}}{
                    \infer[\mathsf{sw}]{{-} \mid A'^{\bullet} , \Gamma^{x} \vdash^{\bullet}_{\RI} P}{
                        \infer[\pass]{{-} \mid A'^{\bullet} , \Gamma^{x} \vdash^{\bullet}_{\Pass} P}{
                            \deduce{A' \mid \Gamma \vdash_{\LI} P}{f}
                        }
                    }
                }
      ...
      \end{array}
    \end{displaymath}
    where $\Gamma_0 ; (A' , \Gamma_1) \bowtie (A' , \Gamma)$ and $(A' , \Gamma_1 \lrwavy \Gamma'_1)$.
    \item $\tr_{\Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g) \defeq \pass \text{ } (\tr_{\Gamma'}^{\Pass} \text{ } (f , g))$, and
    \item $\tr_{\Gamma'}^{\Pass} \text{ } (\F 2 \Pass \text{ } f , g) \defeq \F 2 \Pass \text{ } (\tr_{\Gamma'}^{\F} \text{ } (f , g)) $.
\end{enumerate}
Finally, there are five cases for $\tr_{\Gamma'}^{\F}$:
\begin{enumerate}
  \item $\tr_{\Gamma'_1}^{\F} \text{ } (\ax , g) \defeq \tr \text{ } (\ax , g)$ where $\Gamma'_1$ is empty.
  \item $\tr_{\Gamma'_1}^{\F} \text{ } (\unitr , g) \defeq \tr \text{ } (\unitr , g)$ where $\Gamma'_1$ is empty.
  \item $\tr_{\Gamma'_1}^{\F} \text{ } (\tr \text{ } (f , f') , g) $
  %\defeq \tr \text{ } (\lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{} (\F 2 \Pass \text{ } (\tr \text{ } (f , f'))))) , g)
  \begin{displaymath}
    \footnotesize
    \begin{array}{ll}
    ...
    &
    \proofbox{
    \infer[\tr]{T \mid \Gamma_0 , \Lambda \vdash_{\F} \ldbc \Gamma'_1 \mid A \ot B \rdbc_{\lolli} \ot C}{
        \infer[\lright^{*}]{T \mid \Gamma_0 \vdash^{\bullet}_{\RI} \ldbc \Gamma'_1 \mid A \ot B \rdbc_{\lolli}}{
            \infer[\mathsf{sw}]{T \mid \Gamma^{x} , \Delta^{x} \vdash^{\bullet}_{\RI} A \ot B}{
                \infer[\tr]{T \mid \Gamma^{x} , \Delta^{x} \vdash^{\bullet}_{\F} A \ot B}{
    ...
    \end{array}
  \end{displaymath}
  where $\Gamma_0 ; \Gamma_1 \bowtie (\Gamma , \Delta)$ and $\Gamma_1 \lrwavy \Gamma'_1$.
  \item $\tr_{\Gamma'_1}^{\F} \text{ } (\lleft \text{ } (f , f') , g) \defeq \lleft \text{ } (f , \tr_{\Gamma'_1}^{\LI} \text{ } (f' , g))$ when there is no formula in $\Gamma'_1$ comes from $f$.
  \item When there is at least one formula in $\Gamma'_1$ appears in $f$, then $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f , f') , g)$
  %\defeq \tr \text{ } (\lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\F 2 \Pass \text{ } (\lleft \text{ } (f , f'))))) , g)
  \begin{displaymath}
    \footnotesize
  \begin{array}{ll}
    ...
    &
    \proofbox{
    \infer[\tr]{A' \lolli B' \mid \Gamma_0 , \Lambda \vdash_{\F} \ldbc \Gamma'_1 \mid P \rdbc_{\lolli} \ot B}{
        \infer[\lright^{*}]{A' \lolli B' \mid \Gamma_0 \vdash^{\bullet}_{\RI} \ldbc \Gamma'_1 \mid P \rdbc_{\lolli}}{
            \infer[\mathsf{sw}]{A' \lolli B' \mid \Gamma^{x}_{00} , D^{\bullet} , \Gamma^{x}_{01} , \Delta^{x} \vdash^{\bullet}_{\RI} P}{
                \infer[\lleft]{A' \lolli B' \mid \Gamma^{x}_{00} , D^{\bullet} , \Gamma^{x}_{01} , \Delta^{x} \vdash^{\bullet}_{\F} P}{
                    \deduce{{-} \mid \Gamma_{00} , D , \Gamma_{01}  \vdash_{\RI} P}{f}
                    &
                    \deduce{B' \mid \Delta \vdash_{\LI} B}{f'}
                }
            }
  ...
  \end{array}
   \end{displaymath}
\end{enumerate}
Admissibility of $\tr^{\RI}_{\Gamma'_1}$ is proved, now we show how to connect $\tr^{\C}$ and $\tr^{\RI}_{\Gamma'_1}$.
For $\tr^{\C}$, we do induction on right premise $g$:
\begin{enumerate}
	\item $\tr^{\C} \text{ } (f , \ex \text{ } g) \defeq \ex \text{ } (\tr^{\C} \text{ } (f , g))$, and
	\item $\tr^{\C} \text{ } (f , \RI 2 \C \text{ } g) \defeq \tr^{\C \text{-} \RI} \text{ } (f , g)$.
\end{enumerate}
We prove  $\tr^{\C \text{-} \RI}$ by induction on left premise $f$ :
\begin{enumerate}
	\item $\tr^{\C \text{-} \RI} \text{ } (\ex \text{ } f , g) \defeq \ex \text{ } (\tr^{\C \text{-} \RI} \text{ } (f , g))$, and 
	\item $\tr^{\C \text{-} \RI} \text{ } (\RI 2 \C \text{ } f , g)$
