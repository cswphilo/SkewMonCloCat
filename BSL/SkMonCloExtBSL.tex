...
\begin{document}
	...
	%In this sense, two equivalent proofs in \SkNMILL\ become identical in focused calculus.
	 ...
	 \begin{proof}
		...
		\begin{enumerate}
			\item $\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } f , g) \defeq \tr_{\Gamma' , A}^{\RI} \text{ } (f , g)$ 
			\begin{displaymath}
				\footnotesize
				\begin{array}{c}
				...
				\end{array}
			\end{displaymath}
			where by definition $\ldbc \Gamma' \mid (A \lolli B) \rdbc_{\lolli} = \ldbc \Gamma' , A \mid B \rdbc_{\lolli} $, and
		...
		\begin{enumerate}
			\item $\tr_{A , \Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g) \defeq \F 2 \Pass \text{ } (\tr \text ( \lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\pass \text{ } f))), g))$ when passivated formula is in $\Gamma'$.
			\begin{displaymath}
				\footnotesize
				\begin{array}{c}
		...
		\begin{enumerate}
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\ax , g) \defeq \tr \text{ } (\ax , g)$,
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\unitr , g) \defeq \tr \text{ } (\unitr , g)$,
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\tr \text{ } (f , f') , g) \defeq \tr \text{ } (\lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{} (\F 2 \Pass \text{ } (\tr \text{ } (f , f'))))) , g)$,
		  \begin{displaymath}
			\footnotesize
			\begin{array}{c}
		    ...
		    \end{array}
		  \end{displaymath}
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f , f') , g) \defeq \lleft \text{ } (f , \tr_{\Gamma'}^{\LI} \text{ } (f' , g))$ when there is no formula in $\Gamma'$ comes from $f$, and
		  \item $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f , f') , g) \defeq \tr \text{ } (\lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\F 2 \Pass \text{ } (\lleft \text{ } (f , f'))))) , g)$ when there is at least one formula in $\Gamma'$ appears in $f$.
		  \begin{displaymath}
			\footnotesize
		  \begin{array}{c}
	...
	Following the idea in \cite{veltri:coherence:2021}, we consider a symmetric extension (\SkMILL) of \SkNMILL \ref{eq:seqcalc} by adding an exchange rule where only formulae in context are exchangeable.
...
\begin{proposition}
    ...
    \end{displaymath}
    is admissible, where $\Gamma_1 \lrwavy \Gamma_1'$ means that $\Gamma'_1$ is a permutation of $\Gamma_1$.
\end{proposition}
\begin{proof}
    We prove by induction on $\Gamma'_1$,
    \begin{itemize}
        \item $\Gamma'_1 = [\text{ }]$, then $\lright^{*} \text{ } f \defeq f$;
        \item $\Gamma'_1 = A , \Gamma'_1$, then $\lright^{*} \text{ } f \defeq \lright \text{ } (\ex \text{ } (\RI 2 \C \text{ } (\lright^{*} f)))$, i.e. 
        \begin{displaymath}
            \footnotesize
            \begin{array}{c}
            \infer[\lright^{*}]{S \mid \Gamma_{00} , \Gamma_{01} \vdash^{x}_{\RI} \ldbc A , \Gamma'_1 \mid C \rdbc_{\lolli}}{
                ...
                &
                (\Gamma_{00} , \Gamma_{01}) ; (\Gamma_{10} , A , \Gamma_{11}) \bowtie (\Gamma' , A , \Gamma'')
                &
                (\Gamma_{10} , A , \Gamma_{11}) \lrwavy (A , \Gamma'_1)
            }
            \\[10pt]
            \defeq
            \\[10pt]
                ...
                \infer[\ex]{S \mid A^{x} \spl \Gamma_{00} , \Gamma_{01} \vdash^{x}_{\C} \ldbc \Gamma'_1 \mid C \rdbc_{\lolli}}{
                    \infer[\RI 2 \C]{S \mid \Gamma_{00} , A^{x} , \Gamma_{01} \vdash^{x}_{\C} \ldbc \Gamma'_1 \mid C \rdbc_{\lolli}}{
                        \infer[\lright^{*}]{S \mid \Gamma_{00} , A^{x} , \Gamma_{01} \vdash^{x}_{\RI} \ldbc \Gamma'_1 \mid C \rdbc_{\lolli}}{
                            \deduce{S \mid \Gamma'^{x} , A^{x} , \Gamma''^{x} \vdash^{x}_{\RI} C}{f}
                            &
                            (\Gamma_{00} , \Gamma_{01}) ; (\Gamma_{10} , \Gamma_{11}) \bowtie (\Gamma' , \Gamma'')
                            &
                            (\Gamma_{10} , \Gamma_{11}) \lrwavy \Gamma'_1
                        }
                    }
                }
            }
...
\begin{proof}
    ...
    \begin{equation}
        \footnotesize
        \begin{array}{c}
            \infer[\tr^{\C}]{S \mid \Gamma , \Delta \spl \Lambda \vdc A \ot B}{
                \deduce{S \mid \Gamma \spl \quad \vdc A}{f}
                &
                \deduce{{-} \mid \Delta \spl \Lambda \vdc B}{g}
            }
            \qquad
        ...
        \end{array}
    \end{equation}
    Proof is done by induction on left premises except $\tr^{\C}$, which is on right premise.

    For $\tr^{\C}$, there are two cases:
\begin{enumerate}
	\item $\tr^{\C} \text{ } (f , \ex \text{ } g) \defeq \ex \text{ } (\tr^{\C} \text{ } (f , g))$, and
	\item $\tr^{\C} \text{ } (f , \RI 2 \C \text{ } g) \defeq \tr^{\C \text{-} \RI} \text{ } (f , g)$.
\end{enumerate}
We move to prove  $\tr^{\C \text{-} \RI}$ by induction on left premise $f$ :
\begin{enumerate}
	\item $\tr^{\C \text{-} \RI} \text{ } (\ex \text{ } f , g) \defeq \ex \text{ } (\tr^{\C \text{-} \RI} \text{ } (f , g))$, and 
	\item $\tr^{\C \text{-} \RI} \text{ } (\RI 2 \C \text{ } f , g) \defeq \RI 2 \C \text{ } (\tr_{[\text{ }]}^{\RI} \text{ } (f , g))$.
	\begin{displaymath}
        \footnotesize
        \begin{array}{c}
    ...
    \end{displaymath}
\end{enumerate}
$\tr^{\C}$ and $\tr^{\RI}_{\Gamma'_1}$ is connected, then we show admissibility of $\tr^{ph}_{\Gamma'_1}$ in focused calculus.
There are two cases for $\tr_{\Gamma'}^{\RI}$:
\begin{enumerate}
    \item $\tr_{\Gamma'}^{\RI} \text{ } (\lright \text{ } (\ex \text{ } (\RI 2 \C \text{ } f)) , g) \defeq \tr_{\Gamma' , A}^{\RI} \text{ } (f , g)$ 
    \begin{displaymath}
        \footnotesize
        \begin{array}{c}
            \infer[\tr^{\RI}_{\Gamma'_1}]{S \mid \Gamma_0 , \Delta \vdash_{\RI} \ldbc \Gamma'_1 \mid (A \lolli B) \rdbc_{\lolli} \ot C}{
                ...
                \deduce{{-} \mid \Delta \vdash_{\RI} C}{g}
                &
                \Gamma_0 ; \Gamma_1 \bowtie (\Gamma' , \Gamma'')
                &
                \Gamma_1 \lrwavy \Gamma'_1
            }
            \\[10pt]
            ...
            \infer[\tr^{\RI}_{\Gamma'_1 , A}]{S \mid \Gamma_0 , \Delta \vdash_{\RI} \ldbc \Gamma'_1 \mid (A \lolli B) \rdbc_{\lolli} \ot C}{
                ...
                \deduce{{-} \mid \Delta \vdash_{\RI} C}{g}
                &
                \Gamma_0 ; (\Gamma_{10} , A , \Gamma_{11}) \bowtie (\Gamma' , A , \Gamma'')
                &
                (\Gamma_{10} , A , \Gamma_{11}) \lrwavy (\Gamma'_1 , A)
            }
        \end{array}
    \end{displaymath}
    where by definition $\ldbc \Gamma'_1 \mid (A \lolli B) \rdbc_{\lolli} = \ldbc \Gamma'_1 , A \mid B \rdbc_{\lolli} $, and
...
\end{enumerate}
$\tr_{\Gamma'_1}^{\Pass}$ is a bit more complicated since the passivated formula could appear in $\Gamma'$:
\begin{enumerate}
    \item $\tr_{A , \Gamma'}^{\Pass} \text{ } (\pass \text{ } f , g) \defeq \F 2 \Pass \text{ } (\tr \text ( \lright^{*} \text{ } (\LI 2 \RI \text{ } (\Pass 2 \LI \text{ } (\pass \text{ } f))), g))$ when passivated formula is in $\Gamma'$.
    \begin{displaymath}
        \footnotesize
