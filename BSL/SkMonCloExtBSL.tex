...
%            &
	...
	Following the ideas in Veltri's study, we discuss a commutative extension of \SkNMILL\ \ref{eq:seqcalc}, that we call \SkMILL\ (dropping the \texttt{N} for `non-commutative') and its commection to symmetric skew monoidal closed categories.
        
	\subsection{A Sequent Calculus for \SkMILL}\label{sec:skmill}
        A sequent calculus for \SkMILL\ is obtained by adding the following \emph{restricted} exchange rule to the collection of infererence rules in (\ref{eq:seqcalc}):
...
%	\end{array}
	\end{equation}
        An application of rule $\ex_{A,B}$ results in the swapping of two adjacent formulae $A$ and $B$ in the context. But notice that the exchange of the formula in the stoup (when it exists) and a formula in context is not allowed. This implies that a derivation correponding to the structural law $s$ of skew symmetric monoidal closed categories (see Definition \ref{def:symm-skewcat}) exixts, while a derivation corresponding to the usual symmetry of (non-skew) symmetric monoidal categories does not:
...
\subsection{Focused system with tag annotations}\label{seq:sym-focus}
