...
$B$ and $C$, leaving the first object $A$ in its place.
	...
	\end{itemize}
        ...
        We then introduce a cut-free sequent calculus for a skew variant of \MILL, which we name \SkMILL. The inference rules of the calculus are the one of \SkNMILL\ augmented with an exchange rule that allows the swapping of formulae in context but keeps the formula in the stoup position (whenever there is one) in its place. We show that \SkMILL\ has categorical semantics in the symmetric skew monoidal closed categories of Bourke and Lack \cite{bourke:lack:braided:2020} and it corresponds to a presentation of the free symmetric skew monoidal closed category. We then define a focused sequent calculus for \SkMILL, extending the focused calculus for \SkNMILL with an exchange rule in a way inspired by a Veltri's account of focusing in the non-closed case \cite{veltri:coherence:2021}.

        
%        \paragraph{Formalization}
	The equivalence between sequent calculus derivations in \SkNMILL, quotiented by the equivalence relation $\circeq$, and focused derivations, that we present in Section \ref{sec:focus}, has been formalized in the Agda proof assistant. Also formalized is the focusing procedure for \SkMILL, that we present in Section \ref{sec:sym-focus}. The associated code is available at \url{https://github.com/niccoloveltri/code-skewmonclosed}.

%        \paragraph{Publication History}
        This paper is an extended version of \cite{UVW:protsn}. Compared to the conference version, we have added a detailed proof of the admissibility of rule $\tr^{\RI}_{\Gamma'}$ in the end of Section \ref{sec:focus}, which is the most challenging part in the definition of the focusing algorithm. The whole Section \ref{sec:comm}, studying the proof theory of \SkMILL, is new.
...
\\[6pt]
	 ...
	 To prove the admissibility of $\tr_{\Gamma'}^{\RI}$, it is essential to build a focused variant of the rule $\lright^{*}$ from (\ref{eq:inter:ante}). We detail its construction in the upcoming proposition.
...
\\
		...
		\item If $f = \lleft \text{ } (f' , f'')$, there are two possibilities, depending on whether the $\lleft$ rule splits the context in $\Gamma$ or $\Gamma'$, i.e. whether at least one formula from $\Gamma'$ is moved to the left premise $f'$.                  
                  In the first case $\tr^{\F}_{\Gamma'}$ is permuted with $\lleft$, i.e. $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f' , f'') , g) \defeq \lleft \text{ } (f' , \tr_{\Gamma'}^{\LI} \text{ } (f'' , g))$. % when there is no formula in $\Gamma'$ comes from $f$.
