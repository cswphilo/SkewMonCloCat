...
%                &
		...
		\begin{itemize}
		  ...
		  \item If $f = \tr \text{ } (f' , f'')$, there are two possibilities, depending on whether the $\tr$ rule splits the context in $\Gamma$ or $\Gamma'$.
 ...
 \infer[\tr_{\Gamma_0' ,\Gamma_1'}^{\F}]{T \mid \Gamma , \Delta \vdash_{\F} \ldbc \Gamma_0' , \Gamma_1' \mid A' \ot B' \rdbc_{\lolli} \ot B}{
 	\infer[\tr]{T \mid \Gamma , \Gamma_0' , \Gamma_1' \vdash_{\F} A' \ot B'}{
 		\deduce{T \mid \Gamma , \Gamma_0' \vdash^{\bullet}_{\RI} A'}{f'}
 		&
 		\deduce{{-} \mid \Gamma_1' \vdash_{\RI} B'}{f''}
 	}
 	&
 	\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
 }
\\
		    ...
		    \end{array}
		  \end{displaymath}                  
		\item If $f = \lleft \text{ } (f' , f'')$, there are two possibilities, depending on whether the $\lleft$ rule splits the context in $\Gamma$ or $\Gamma'$, i.e. whether at least one formula from $\Gamma'$ is moved to the left premise $f'$.                  
                  In the first case, $\lleft$ can be applied first, i.e. $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f' , f'') , g) \defeq \lleft \text{ } (f' , \tr_{\Gamma'}^{\LI} \text{ } (f'' , g))$. % when there is no formula in $\Gamma'$ comes from $f$.
...
%		  \item  When there is at least one formula in $\Gamma'$ appears in $f$, $\tr_{\Gamma'}^{\F} \text{ } (\lleft \text{ } (f , f') , g)$
 ...
 \infer[\tr_{\Gamma'_{0} , \Gamma'_1}^{\F}]{A' \lolli B' \mid \Gamma , \Delta \vdash_{\F} \ldbc \Gamma'_{0} , \Gamma'_1 \mid P \rdbc_{\lolli} \ot B}{
 	\infer[\lleft]{A' \lolli B' \mid \Gamma , \Gamma'_0 , \Gamma'_1 \vdash_{\F} P}{
 		\deduce{{-} \mid \Gamma , \Gamma'_0  \vdash_{\RI} A'}{f'}
 		&
 		\deduce{B' \mid \Gamma'_1 \vdash_{\LI} P}{f''}
 	}
 	&
 	\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
 }
...
\begin{itemize}
\item Root-first proof search now starts in a new phase $\C$, where formulae in context are permuted. The search in phase $\C$ starts from a sequent $S \mid \Gamma \spl ~ \vdash_{\C} C$ and ends in a sequent $S \mid ~ \spl \Gamma' \vdash_{\C} C$, where $\Gamma'$ is a permutation of $\Gamma$.
  ...
  The rule $\ex$ moves one formula at a time (the rightmost one in the left side of the context $\Gamma$) until every formula is moved somewhere to the right of the separator $\spl$. When this happens, proof search continues in phase $\RI$. Notice that the sequent and formulae in context may also be tagged and tags are preserved during the permutation phase. An important difference with the  non-commutative focused calculus is the fact that now tagged and untagged formulae in context are mixed, while in (\ref{eq:focus}) all tagged formulae necessarily appear at the right end of the context.
  \item The other difference is in rules $\lright$ and $\tl$, which, under the bottom-up reading of the rules, are the only two rules where new formulae are added to the context. The premises of these rules are now required to go back to phase $\C$ in order to move the new formulae $A$ to its correct position in the context via an application of the rule $\ex$.
\end{itemize}
...
%\begin{proof}
  ...
  \begin{equation*}\label{eq:admis:sym}
    ...
    \\[6pt]
        \infer[\tr^{\C}]{S \mid \Gamma , \Gamma'  \spl \Delta \vdash_{\C} A \ot B}{
          S \mid \Gamma \spl ~  \vdash_{\C} A
          &
          {-} \mid \Gamma' \spl \Delta \vdash_{\C} B
        }
        \quad
...
%   \end{align*}
  Theorem \ref{thm:focus:sym} is then proved by showing the following three properties, proving that the set of derivations of $S \mid \Gamma \vdash A$ quotiented by $\circeq$ is in bijection with the set of derivations of $S \mid \Gamma \spl ~ \vdc A$:
  \begin{itemize}
...
%  The second and the last entail that $\mathsf{focus}$ and $\mathsf{emb}_{\C}$ are inverses of each other.
  ...
  The focused sequent calculus can be used to decide whether two morphisms in the free symmetric skew monoidal closed category are ``equal'', in the sense of being related by the equivalence relation generated by the equational theory of symmetric skew monoidal closed categories. In the non-closed case, Veltri~\cite{veltri:coherence:2021} showed that the coherence problem has a more interesting solution than the one for non-skew symmetric monoidal categories. In fact, there exist pairs of non-equal morphisms which have the same underlying permutation of atomic formulae, a phenomenon that is peculiar to the skew case and is not present in the usual non-skew case. Veltri illustrates this by showing two distinct focused derivations of the sequent ${-} \mid X,\I \ot Y \vdash_{\C} X \ot Y$. Clearly, these derivations can also be replicated in the focused sequent calculus (\ref{eq:focus:sym}) for \SkMILL.
  ...
  To this end, we introduce some notation. We write $\Gamma \lrwavy \Gamma'$ when $\Gamma$ is a permutation of $\Gamma'$. We write $\Gamma_0 ; \Gamma_1 \bowtie \Gamma$ if the context $\Gamma$ is equal to an interleaving of $\Gamma_0$ and $\Gamma_1$, which can be formally described as the following inductive relation:
    \begin{displaymath}
%        \begin{array}{cc}
            \infer{\Gamma_0 ; ~ \bowtie \Gamma_0}{}
            \quad
            \infer{~ ; \Gamma_1 \bowtie \Gamma_1}{}
            %            \\[10pt]
            \quad
...
%     \end{displaymath}
% \end{lemma}
    Assume $\Gamma_0 ; \Gamma_1 \bowtie \Gamma$. If $\Gamma$ is empty then both $\Gamma_0$ and $\Gamma_1$ are forced to be empty. If $\Gamma = A,\Gamma'$ then $A$ is either the leftmost formula of $\Gamma_0$ or the leftmost formula of $\Gamma_1$. Crucially, the relative position of formulae in $\Gamma_0$ is preserved in $\Gamma$ and the same is true for $\Gamma_1$. 
When $\Gamma_0;\Gamma_1 \bowtie \Gamma$, we write $\Gamma^{\Gamma^{x}_1}$ to indicate the context $\Gamma$ in which all formulae in $\Gamma_1$ have been assigned tag $x$ and all formulae in $\Gamma_0$ are kept untagged.
%%     \begin{definition}
...
%% \end{definition}
    ...
    \begin{displaymath}
        \infer[\lright^{*}]{S \mid \Gamma_0 \vdash^{x}_{\RI} \ldbc \Gamma'_1 \mid C \rdbc_{\lolli}}{
            S \mid \Gamma^{\Gamma^{x}_{1}} \vdash^{x}_{\RI} C
            &
    ...
    \end{displaymath}
	%$\Gamma^{\Gamma^{x}_{1}}$ indicates the context $\Gamma$ in which all formulae $\Gamma_{1}$ are possibly tagged and all formulae in $\Gamma_{0}$ are kept untagged.
\end{proposition}
\begin{proof}
...
\proofbox{ \infer[\lright^{*}]{S \mid \Gamma_{00} , \Gamma_{01} \vdash^{x}_{\RI} \ldbc A , \Gamma''_1 \mid C \rdbc_{\lolli}}{
     \deduce{S \mid (\Gamma')^{\Gamma^{x}_{10}} , A^{x} , (\Gamma'')^{\Gamma^{x}_{11}} \vdash^{x}_{\RI} C}{f}
     &
 }
 }
%\\
...
%          we extend $\iota : \Gamma_0 ; (\Gamma_{10} , \Gamma_{11}) \bowtie (\Gamma' , \Gamma'')$ and $(\Gamma_{10} , \Gamma_{11}) \lrwavy \Gamma'_1$ with $A$, the active formula in $\lright \text{ } (\ex \text{ } (\RI 2 \C \text{ } f'))$, then we have $\iota' : \Gamma_0 ; (\Gamma_{10} , A , \Gamma_{11}) \bowtie (\Gamma' , A , \Gamma'')$, $(\Gamma_{10} , A , \Gamma_{11}) \lrwavy (\Gamma'_1 , A)$ and
			...
			\begin{displaymath}
				...
				\begin{array}{c}
					\hspace*{-3cm}
				    \proofbox{
				    \infer[\tr^{\RI}_{\Gamma'_1}]{S \mid \Gamma_0 , \Delta \vdash_{\RI} \ldbc \Gamma'_1 \mid A' \lolli B' \rdbc_{\lolli} \ot B}{
				        \infer[\lright]{S \mid \Gamma' , \Gamma'' \vdash_{\RI} A' \lolli B'}{
				...
				\\
					\defeq
				\proofbox{
					\infer[\tr^{\RI}_{\Gamma'_1 , A'}]{S \mid \Gamma_0 , \Delta \vdash_{\RI} \ldbc \Gamma'_1 ,A' \mid B' \rdbc_{\lolli} \ot B}{
						\deduce{S \mid \Gamma' , A' , \Gamma'' \vdash_{\RI} B'}{f'}
						&
						\deduce{{-} \mid \Delta \vdash_{\RI} B}{g}
					}
	...
	\begin{itemize}
	\item If $f = \pass \text{ } f'$, the passivated formula $A'$ can either belong to $\Gamma_0$ or to $\Gamma_1$, which follows from the assumption $\Gamma_0 ; \Gamma_1 \bowtie A' , \Gamma$. In the first case $\Gamma_0 = A',\Gamma_0'$ and $\pass$ can be applied first, i.e.
...
%		\end{itemize}
	...
	\begin{itemize}
	  ...
	  \begin{displaymath}
	    ...
	    \hspace*{-3cm}
		\proofbox{
		\infer[\tr_{\Gamma'_1}^{\F}]{T \mid \Gamma_{00} , \Gamma_{01} , \Delta \vdash_{\F} \ldbc \Gamma'_1 \mid A' \ot B' \rdbc_{\lolli} \ot B}{
		...
		\\
			\defeq
		\proofbox{
		\infer[\tr]{T \mid \Gamma_{00} , \Gamma_{01} , \Delta \vdash_{\F} \ldbc \Gamma'_1 \mid A' \ot B' \rdbc_{\lolli} \ot B}{
...
%	  \begin{itemize}
	  If $\Gamma_{10}$ is empty, then the $\lleft$ can be applied first, i.e. $\tr_{\Gamma'_1}^{\F} \text{ } (\lleft \text{ } (f' , f'') , g) \defeq \lleft \text{ } (f' , \tr_{\Gamma'_1}^{\LI} \text{ } (f'' , g))$.
%          
...
(\Gamma')^{\Gamma_{10}^{\bullet}} , (\Gamma'')^{\Gamma^{\bullet}_{11}} \vdash^{\bullet}_{\F} P}{
		  ...
		  \end{array}
		   \end{displaymath}
%	  \end{itemize}
                 The last application of $\lleft$ is justified since $\Gamma_{10}$ is non-empty.
	\end{itemize}
\end{proof}
\begin{proposition}
    ...
    \begin{displaymath}
		...
		\qquad
		\infer[\tr^{\C\text{-}\RI}]{S \mid \Gamma \spl \Delta , \Delta' \vdc A \ot B}{
...
\end{proof}
	...
	We plan to investigate alternative presentations of \SkNMILL\ and \SkMILL, such as natural deduction. Analogously to the case of the sequent calculi studied in this paper, we expect natural deduction derivation to be strongly normalizing wrt.\ an appropriately defined $\beta\eta$-conversion. We are interested in directly comparing the resulting $\beta\eta$-long normal forms with the focused derivations of Section \ref{sec:focus} and Section \ref{sec:sym-focus}.
	...
	There exist other techniques for drastically reducing non-determinism in proof search, such as maximal multi-focusing \cite{chaudhuri:canonical:2008} and saturated focusing \cite{scherer:simple:2015}, and we wonder if our system of tags is in any way related to these. We also wonder whether the device of tags can be employed in the design of a calculus of maximally multi-focused derivations in classical linear logic, providing an explicit descriptions of the normal forms in \cite{chaudhuri:canonical:2008}, which will then be equivalent to proof nets.  As a general disclaimer, we should state that we have interpreted focusing broadly as the idea of root-first proof search defining a normal form, based on a careful discipline of application of invertible and non-invertible rules to reduce non-determinism, but not necessarily driven by a polarity-centric analysis. In this respect, our approach is similar in spirit to, e.g., \cite{dyckhoff:ljq}.
