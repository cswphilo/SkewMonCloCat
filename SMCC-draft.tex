...
\section{A Sequent Calculus for Skew Noncommutative MILL}
...
These two features present differences between $A \mid \Gamma$ and $- \mid \Gamma$ which are form of antecedents and perform distinctions with $\I$, $\ot$, $\lolli$ fragment of NMILL.
...
\niccolo{
  ...
  \begin{fact}
    Folloing rules are admissible in SkNMILL:
    \begin{displaymath}
    ...
    \end{displaymath}
    Inductive definitions of formulae $\ldbc A \mid \Gamma \rdbc_{\ot}$ (defined as $\ldbc S \mid \Gamma \rdbc_{\ot} = \ldbc S \llangle \text{ } \llangle \Gamma \rdbc_{\ot}$) and $\ldbc \Gamma \mid C \rdbc_{\lolli}$ are from \cite{uustalu:sequent:2018} and \cite{uustalu:deductive:nodate}, respectively:
    \begin{align*}
...
(a1) and (a8) are presented in hom-set form in original paper but they are rewritten into internal hom presentations here.
Notice that morphism $(A \ot \I) \lolli B \Longrightarrow A \lolli (\I \lolli B)$ in (a8) is actually an isomorphism in hom-set presentation, but only one direction left in the form internal hom.
In general, this facts means that we \emph{cannot} internalize both directions of  adjunction bijection in a skew monoidal closed category.
It requires our skew category to at least be associative normal, which means that natural transformation $\alpha$ is an isomorhphism, then we can internalize both directions of adjunction bijection.
...
\begin{proof}
  ...
  \end{displaymath}
\end{proof}
\begin{theorem}\label{thm:wkcomplete}
