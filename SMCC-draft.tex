...
Thanks for the invertibility of $\tl$ and $\lright$ \cite{uustalu_sequent_2018} \cite{uustalu_deductive_nodate}, two adjoint rules are also interpretable in sequent calculus system:
\begin{center}
  \AxiomC{$f$}
  \noLine
...
\begin{center}
\AxiomC{$S \mid \Gamma , A \vdash^{x}_{\mathsf{RI}} B$}
\LeftLabel{Phase $\mathsf{RI}$: }
\RightLabel{$\multimap \mathsf{R}$}
\UnaryInfC{$S \mid \Gamma \vdash^{x}_{\mathsf{RI}} A\multimap B$}
\DisplayProof
\quad
\AxiomC{$S \mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\RightLabel{$\mathsf{LI2RI}$}
\UnaryInfC{$S \mid \Gamma \vdash_{\mathsf{RI}} P$}
\DisplayProof
\end{center}

\begin{center}
\AxiomC{$- \mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\LeftLabel{Phase $\mathsf{LI}$: }
\RightLabel{$\mathsf{IL}$}
\UnaryInfC{$\mathsf{I} \mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\DisplayProof
\quad
\AxiomC{$A \mid B, \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\RightLabel{$\otimes \mathsf{L}$}
\UnaryInfC{$A\otimes B \mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\DisplayProof
\end{center}

\begin{center}
\AxiomC{$T \mid \Gamma \vdash^{x}_{\mathsf{P}} P$}
\LeftLabel{Phase $\mathsf{LI}$: }
\RightLabel{$\mathsf{P2LI}$}
\UnaryInfC{$T \mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
\DisplayProof
\end{center}

\begin{center}
  \AxiomC{$A\mid \Gamma \vdash^{x}_{\mathsf{LI}} P$}
  \LeftLabel{Phase $\mathsf{P}$: }
  \RightLabel{$\mathsf{pass}$}
  \UnaryInfC{$- \mid A, \Gamma \vdash^{x}_{\mathsf{P}} P$}
  \DisplayProof
  \quad
  \AxiomC{$T \mid \Gamma \vdash_{\mathsf{F}} P$}
  \RightLabel{$\mathsf{F2P}$}
  \UnaryInfC{$T \mid \Gamma \vdash^{x}_{\mathsf{P}} P$}
\end{center}

\begin{center}
\AxiomC{$- \mid \Gamma_{0} \vdash_{\mathsf{RI}} A$}
\AxiomC{$B \mid \Gamma_{1} \vdash^{x}_{\mathsf{LI}} P$}
\AxiomC{$B \neq T$ or $C \neq A' \otimes B'$}
\RightLabel{$\multimap \mathsf{L}$}
\LeftLabel{Phase $\mathsf{F}$:}
\TrinaryInfC{$A\multimap B \mid \Gamma_{0} , \Gamma_{1} \vdash_{\mathsf{F}} C$}
\DisplayProof
\end{center}
%\begin{center}
...
%\BinaryInfC{$S \mid \Gamma \vdash_{\mathsf{F}} C$}
%\DisplayProof
%\end{center}

\begin{center}
\AxiomC{$T\mid \Gamma \vdash_{\mathsf{RI}} A$}
\AxiomC{$- \mid \Delta \vdash_{\mathsf{RI}} B$}
\RightLabel{$\otimes \mathsf{R}$}
\LeftLabel{Phase $\mathsf{F}$: }
\BinaryInfC{$T \mid \Gamma , \Delta \vdash_{\mathsf{F}} A\otimes B$}
\DisplayProof
\quad
\AxiomC{}
\RightLabel{$\mathsf{ax}$}
\UnaryInfC{$A \mid \quad \vdash_{\mathsf{F}} A$}
\DisplayProof
\end{center}
\begin{center}
\AxiomC{}
...
\LeftLabel{Phase $\mathsf{F}$: }
\UnaryInfC{$- \mid \quad \vdash_{\mathsf{F}} \mathsf{I}$}
\DisplayProof
\end{center}
$T$ in stoup means that $T$ is irreducible(empty or atom).
$P$ in conclusion means that $P$ is positive(not in $A \multimap B$).
Notice that our interpretation $\ldbc - \rdbc_{s}$ is same as the definition in \cite{uustalu_deductive_nodate}.
