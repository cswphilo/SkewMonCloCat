...
Derivations in the focused sequent calculus are generated by the rules
%\begin{defn}
...
\end{displaymath}
%\end{defn}
In the above rules, the metavariable $P$ denotes a \emph{positive} formula, i.e. $P \not= A \lolli B$, while metavariable $T$ indicates a \emph{negative} stoup, i.e. $T \not= \I$ and $T\not= A \ot B$. The side condition in rule $\lleft$ reads: if $x = \bullet$, then there exists an occurrence of a tagged formula $D^\bullet$ in $\Gamma$.
...
We explain the rules of the focused sequent calculus from the perspective of bottom-up proof search. We start by analyizing the proof search of an untagged sequent $S \mid \Gamma \vdash_{\RI} C$.
...
\item[($\LI$)] We eagerly destruct the stoup formula via application of left invertible rules $\tl$ and $\unitl$ with the goal of making it negative. When this happens we move to phase $\Pass$.
  ...
  \item[($\Pass$)] where only $\pass$ can be applied to sequents.
  ...
  \item[($\F$)] we have four rules, $\tr$, $\lleft$, $\ax$, and $\unitr$.
...
\end{displaymath}
then we can see what happens if we do not endorse a tagged focused system.

Assume that we prefer to decompose $\lolli$ in stoup prior than $\ot$ in succedent without tag setting above.
      ...
      \infer[\mathsf{sw}]{Y \mid Z \vdash_{\LI} (X \lolli Y) \ot Z}{
        \infer[\tr]{Y \mid Z \vdash_{\F} (X \lolli Y) \ot Z}{
          ...
          \infer[\mathsf{sw}]{- \mid Z \vdash_{\RI} Z}{
            \infer[\pass]{- \mid Z \vdash_{\Pass}}{
              \infer[\mathsf{sw}]{Z \mid \quad \vdash_{\LI}}{
