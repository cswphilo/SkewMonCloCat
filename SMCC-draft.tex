...
%}
\begin{defn}
A \emph{skew monoidal closed category} $\mathbb{C}$ is a category with a unit $\I$, and two functors $\ot : \mathbb{C} \times \mathbb{C} \rightarrow \mathbb{C}$ and $\lolli : \mathbb{C}^{\mathsf{op}} \times \mathbb{C} \rightarrow \mathbb{C}$, with an adjunction $- \ot B \dashv B \lolli -$ natural in $B$,
and three natural transfomations
\begin{displaymath}
  \lambda : \I \ot A \Longrightarrow A \qquad \rho : A \Longrightarrow A \ot \I \qquad \alpha : (A \ot B) \ot C \Longrightarrow A \ot (B \ot C)
\end{displaymath}
satisfying following Mac Lane axioms (m1)-(m5)
\begin{center}
  (m1)
