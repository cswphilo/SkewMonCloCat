...
It is a widely known fact from the late 80s/early 90s that symmetric monoidal closed categories are a categorical model of \MILL, multiplicative intuitionistic linear logic, whose logical connectives comprise of multiplicative unit $\I$ and conjunction $\ot$ and linear implication $\lolli$ \cite{mellies:categorical:09}. Probably lesser-known, though a quite straightforward variation of this observation (which actually predated the inception of linear logic altogether) is the fact that (not necessarily symmetric) monoidal biclosed categories are a categorical model of the non-commutative variant of \MILL\ (for which we use the abbreviation \NMILL) \cite{abrusci:noncommutative:1990}, where the structural rule of exchange is absent and there are two ordered implications $\lolli$ and $\illol$ \cite{lambek:deductive:68}. The sequent calculus of \NMILL\ is known as \emph{Lambek calculus}. Lambek refers to the implications $\lolli$ and $\illol$ as residuals, and monoidal biclosed categories are therefore also called residual categories. The Lambek calculus is often practically employed in formal investigations of natural languages \cite{lambek:mathematics:58}.
...
as a theory of resources. Under this perspective, formulae of the
sequent calculus introduce in (\ref{eq:seqcalc}) correspond to
resources: atomic formulae are primitive resources; the formula $A \ot
B$ is read as ``resource $A$ before resource $B$'', and a similar
interpretation is reserved for the separators $\mid$ and comma in the
antecedent; $\I$ is the disposable resource; the formula $A \lolli B$
can be described as a method for turning the resource $A$ into the
resource $B$. As in other substructural systems lacking the structural
...
appears in the antecedent on the left of another resource $B$, then
$A$ must be consumed before $B$. The stoup position, when it is
non-empty, contains the resource at hand at the current time. The
other existing resources in the context will become available only at
future times, after the resource in the stoup is consumed. Time flows
...
premise, in a way reminiscent of the preorder traversal of a tree.

The context shares similarities with the queue and stack data
structures of computer science. If at the current time there is no
manipulable resource, meaning that the stoup is empty, then we the
next available resource at the top (left-most position) of the context
can be popped via the $\pass$ rule and consequently moved to the
stoup. A new resource can be pushed to the bottom (right-most
position) of the context using the $\lright$ rule. Using the $\tl$
rule, the currently given resource can be split in two parts and the
right part can be pushed to the top of the context.  The resource $\I$
can be discarded, making it possible to pop the next available
resource from the context. The resource $\I$ can always be produced
free-of-charge via the rule $\unitr$.

In the rule $\lleft$, we are currently given a resource of the form $A
\lolli B$. Following the application of the rule, we are guaranteed
access to a resource $B$, but only after the resource $A$ is shown to
be obtainable by spending part of the resources that are present in
the context. The production of $A$ is the first event that is required
to happen, only subsequently $B$ will become available. This implies
that we first split the context in two parts and use the left one
right away for making $A$. Immediately afterwards, we are allowed to
continue our derivation with $B$ available at the current time (which
is a future time wrt. the time when $A$ is produced) and the
remaining resources resulting from the context split.

The succedent of the rule $\tr$ is of the form $A \ot B$, which
implies that $A$ must be produced before $B$. This justifies the
splitting of the context in two parts: the left one, consisting of
more readily available resources, is used to produce $A$ in the left
premise, while the right one is spent for the production of $B$ in the
right premise. Crucially, and this is one the central ``skew'' aspect
of \SkNMILL, if we have a resource at hand at the current time in the
stoup, this must be spent for the construction of the first resource
we are required to produce, namely $A$.

The other main ``skew'' aspect of \SkNMILL\ is the fact that left
rules only act on stoup formulae. This can be understood under the
resource-as-formula correspondence by saying that we are allowed to
inspect the shape of one resource at a time, namely the resource in
the stoup. In particular, we are precluded from inspecting the
resources in the context. The latterbecome visible to us only after
...
\hspace*{-5mm}
    ...
    \normalsize
  \end{equation*}
  \item[2.] In phase $\F$, if the succedent formula is $A \ot B$, and the rule $\tr$ is to be applied, the context can be split anywhere. Sometimes, in particular when in the middle of the context there are closed formulae (i.e., formulae made of $\I$ and $\ot$ only), then several of these splits can lead to a derivation.
...
There exist other techniques for drastically reducing non-determinism in proof search, such as multi-focusing \cite{chaudhuri:canonical:2008} and saturated focusing \cite{scherer:simple:2015}, and we wonder if our system of tags is in any way related to these. % other techniques.
